...
@implementation GameViewController
{
    ...
    //auto pTest;
    std::unique_ptr<TestApp> mTestApp;
...
- (void)viewDidLoad
{
    ...
    if(mTestApp == nullptr)
    {
        mTestApp.reset(new TestApp);
    }
    
    
    // Setup up our OSEngine
    auto pConstruct = (IOSNativeApp::PGEConstruction)[app pIOSNativeApp]->structPGEConstruction;
    mTestApp->pOsEngine.screenWidth = (int32_t)widthInPixel;
    mTestApp->pOsEngine.screenHeight = (int32_t)heightInPixel;
    mTestApp->pOsEngine.bMaintainAspectRatio = pConstruct.bMaintainAspectRatio;
    mTestApp->pOsEngine.strInternalAppStorage = [bundlePath UTF8String];
    mTestApp->pOsEngine.strExternalAppStorage = [libraryDirectory UTF8String];
    mTestApp->pOsEngine.strPublicAppStorage = [documentDirectory UTF8String];

    mTestApp->Construct(pConstruct.nScreen_width, pConstruct.nScreen_height, pConstruct.nPixel_width, pConstruct.nPixel_height, pConstruct.bUseFullScreen, pConstruct.bUseVSYNC, pConstruct.bCohesion);
   
    // Set our content scale factor for when full screen is enabled
    if(mTestApp->pOsEngine.bFullScreen == true)
    {
        float customScale = mTestApp->pOsEngine.fScaleFactor; //[UIScreen mainScreen].scale / [UIScreen mainScreen].scale;
...
- (void)setupGL
{
    ...
    // Prepare our Engine:
    mTestApp->olc_PrepareEngine();
    mTestApp->pOsEngine.animating = 0;
    mTestApp->pOsEngine.StartPGE = false;
...
- (void)update
{
    ...
    mTestApp->pOsEngine.animating = 1;
    mTestApp->pOsEngine.StartPGE = true;
}

- (void)glkView:(GLKView *)view drawInRect:(CGRect)rect
{
    [EAGLContext setCurrentContext:self.context];
    mTestApp->olc_CoreUpdate();
    ...
    for (int i=0; i<[allTouches count]; i++)
    {
        ...
        for(int j=0; j<MAX_TOUCHES; j++)
        {
            if(savedTouches[j] == NULL)
            {
                ...
                mTestApp->olc_UpdateTouchState(j, true);
                mTestApp->olc_UpdateTouch(currentPoint.x * fScaleFactor / nTouchScalerX, currentPoint.y * fScaleFactor / nTouchScalerY, j);
                break;
            }
        }
    ...
    for (int i=0; i<[allTouches count]; i++) {
        ...
        for (int j=0; j<MAX_TOUCHES; j++) {
            if (touch == savedTouches[j]) {
                mTestApp->olc_UpdateTouchState(j, false);
                savedTouches[j] = NULL;
...
-(void)pressesBegan:(NSSet<UIPress *> *)presses withEvent:(UIPressesEvent *)event
{
    ...
    {
        mTestApp->olc_UpdateKeyState((int32_t)press.key.keyCode, true);
...
-(void)pressesEnded:(NSSet<UIPress *> *)presses withEvent:(UIPressesEvent *)event
{
    for(UIPress *press in presses)
    {
        mTestApp->olc_UpdateKeyState((int32_t)press.key.keyCode, false);
